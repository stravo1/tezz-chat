<template>
  ...
  >
    <div class="h-[40vh] w-[55vw]">
      ...
      <div class="h-[33vh] overflow-y-auto">
        <div class="flex list-none flex-col p-0">
          <NuxtLink
            v-for="thread in matchedThrads"
            :key="thread.id"
            :to="`/chat/${thread.id}`"
            class="bg-secondary-container/10 text-on-secondary-container hover:bg-secondary-container/30 cursor-pointer border-b p-4"
          >
            {{ thread.title }}
          </NuxtLink>
        </div>
